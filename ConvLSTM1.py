...
train_gen = TimeseriesGenerator(scaled, scaled[:,3], start_index = 0, end_index = int(len(scaled) * 0.8), length = 3, batch_size = 256)
test_gen = TimeseriesGenerator(scaled, scaled[:,3], start_index = int(len(scaled) * 0.8), end_index = int(len(scaled) - 1), length = 3, batch_size = 256)
...
'''
model.add(Conv1D(18, kernel_size=3, activation='relu', padding = 'valid', strides=1, input_shape=(None, 1, 1,18), data_format='channels_first'))
...
##### TRAINING #####
history = model.fit_generator(train_gen, epochs = 100, verbose = 2, shuffle = False, validation_data = test_gen)
...
with open("model_" + ticker + "conv.json", "w") as json_file:
    json_file.write(model_json)
model.save_weights("model_" + ticker + "conv.h5")
