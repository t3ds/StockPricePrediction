...
from keras.layers import Dense, Dropout, Flatten, Conv1D, MaxPooling1D, LSTM,ConvLSTM2D
...
def plot_them_graphs(actual, predicted, type, ticker):
    ...
    #fig1 = plt.gcf()
    plt.show()
    plt.savefig(ticker + '_' + type + "2.png", transparent=False)
...
##### MODEL CREATION ######
model = Sequential()
'''model.add(Conv1D(18, kernel_size=3, activation='sigmoid', padding = 'valid', strides=1, input_shape=(1,18), data_format='channels_first'))
model.add(Conv1D(18, kernel_size=3, activation='relu', padding = 'valid', strides=1))
#model.add(MaxPooling1D(pool_size=2))
model.add(LSTM(100, activation='tanh', return_sequences=True, recurrent_dropout = 0.2))
model.add(LSTM(100, activation='tanh', return_sequences=True, recurrent_dropout = 0.2))
model.add(LSTM(100, activation='tanh', recurrent_dropout = 0.2))
model.add(Dense(1, activation = 'linear'))'''
model = Sequential()
model.add(ConvLSTM2D(filters = 18, kernel_size=(3, 3), input_shape = (1,18), padding='valid', return_sequences= True, data_format = 'channels_first'))
...
##### TRAINING #####
history = model.fit_generator(train_gen, epochs = 1200, verbose = 2, shuffle = False, validation_data = test_gen)
