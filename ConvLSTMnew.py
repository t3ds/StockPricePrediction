...
from tensorflow.keras.layers import Dense, Dropout, Flatten, Conv1D, MaxPooling1D, LSTM, Reshape
...
def plot_them_graphs(actual, predicted, type, ticker):
    ...
    plt.legend()
    #fig1 = plt.gcf()
    plt.savefig(ticker + '_' + type + "2.jpg", transparent=False)
    plt.show()
...
train_gen = TimeseriesGenerator(scaled, scaled[:,3], start_index = 0, end_index = int(len(scaled) * 0.8), length = 1, batch_size = 256)
test_gen = TimeseriesGenerator(scaled, scaled[:,3], start_index = int(len(scaled) * 0.8), end_index = int(len(scaled) - 1), length = 1, batch_size = 256)

##### MODEL CREATION ######
model = Sequential()
model.add(Conv1D(18, kernel_size=3, activation='relu', padding = 'causal', strides=1, input_shape=(1,18), data_format='channels_first'))
model.add(Conv1D(18, kernel_size=3, activation='relu', padding = 'causal', strides=1))
model.add(MaxPooling1D(pool_size=2))
model.add(Flatten())
model.add(Reshape((1,162)))
...
##### TRAINING #####
history = model.fit_generator(train_gen, epochs = 2000, verbose = 2, shuffle = False, validation_data = test_gen)
...
with open("model_" + ticker + "2.json", "w") as json_file:
    json_file.write(model_json)
model.save_weights("model_" + ticker + "2.h5")
